...
- `.agents/skills/ship/SKILL.md`: 단순 type 나열을 동일한 9개 type 분류표와 선택 가이드로 확장 — bug+test 한 commit이면 `fix`, refactor 중 사소한 bug 함께 고치면 `fix`, agent 동작에 영향 주는 메타 문서는 `chore`
