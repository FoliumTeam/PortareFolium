...
[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/FoliumTeam/PortareFolium&env=PUBLIC_SUPABASE_URL,PUBLIC_SUPABASE_ANON_KEY,SUPABASE_SERVICE_ROLE_KEY,PUBLIC_COLOR_SCHEME,PUBLIC_JOB_FIELD&envDescription=Supabase%20인증%20정보와%20사이트%20초기%20설정값.%20아래%20README%를%20참고.&envLink=https://github.com/FoliumTeam/PortareFolium&project-name=folium-online&repository-name=folium-online)
...
| `PUBLIC_COLOR_SCHEME`       | `gray` / `blue` / `beige` / `blackwhite` (기본값: `gray`) |
| `PUBLIC_JOB_FIELD`          | `web` 또는 `game` — 포트폴리오 노출 분야 (기본값: `game`) |
...
`/admin`에 로그인해 About me, 포트폴리오, 블로그 글을 채우면 됨. Color Scheme은 Admin → **Site Config**에서 언제든 변경 가능.
