...
`supabase/setup.sql`을 한 번 실행해 초기화하면, 이후 버전 업데이트에 필요한 DB 마이그레이션은 서버에서 자동 실행됨. 앱은 서버 시작 시 현재 `db_schema_version`을 확인하고, `SUPABASE_SECRET_KEY`로 service-role 전용 `exec_sql()` RPC를 호출해 누락된 마이그레이션을 순서대로 적용함. 따라서 일반적인 업데이트에서는 Admin 화면에서 별도 마이그레이션 패널을 찾아 실행할 필요가 없음.

- 초기 설치: `supabase/setup.sql`은 여전히 SQL Editor에서 1회 실행해야 함.
- 업데이트: 초기화된 DB라면 앱 배포/서버 시작 후 자동 마이그레이션됨.
- 수동 복구/검토: 개별 마이그레이션 SQL은 `supabase/migrations/001_*.sql`부터 순서대로 보관됨.
- Admin 메인 화면의 DB 상태 카드는 현재 DB 버전과 프론트엔드 버전을 표시함.
