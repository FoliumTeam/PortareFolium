...
- **Admin Customization Acceptance Gate**: 신규 기능 완료 전 사용자에게 노출되는 정보와 표현 요소를 전수 확인하고, Admin 로그인만으로 값 입력·수정·삭제·초기화·공개 설정·지원 디자인 선택까지 가능한지 검증한다. 코드 수정, CLI, DB 직접 조작, MCP, AI agent가 필요한 사용자 설정 경로가 남아 있으면 기능 미완료로 판정한다. AI·MCP는 선택적 자동화 수단으로만 허용한다.
- **Preset-First Design Gate**: 사용자 디자인 선택은 2~4개의 완성형 preset과 명확한 preview·추천 기본값을 우선 제공한다. 간격·폭·정렬·색상 같은 저수준 control을 기본 설정 화면에 노출하지 않는다. preset으로 해결할 수 없는 반복적 사용자 요구만 별도 control 후보다.
- **Admin Control Guidance Gate**: 모든 Admin 입력·button·toggle·preset에는 사람이 읽을 수 있는 label과 짧은 목적·입력 형식·공개 효과 설명을 제공한다. Custom preset은 사용 목적과 추가 입력 항목을 명시한다. 순서 변경 control은 이동 방향과 공개 화면 영향 설명을 함께 제공한다. icon-only control은 visible tooltip과 `aria-label` 필수.
- **Automatic Task Commit**: After completing and verifying each task, invoke the project-local [`$ship` Skill](../skills/ship/SKILL.md) automatically to create focused commits for that task's changes. This locator is the sole commit Skill authority for the project. Never push automatically; push requires the user's explicit authorization. Preserve pre-existing or unrelated worktree changes outside the completed task's scope.
