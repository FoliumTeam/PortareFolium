...
- **Admin Customization Acceptance Gate**: 신규 기능 완료 전 사용자에게 노출되는 정보와 표현 요소를 전수 확인하고, 각 정보의 값과 표시 여부를 Admin Dashboard에서 설정할 수 있는지 검증한다. 같은 성격의 항목이 여러 개면 순서 설정을 제공하고, 날짜 형식·label·layout·design처럼 의미 있는 표현 선택지가 있으면 지원 범위를 Admin 설정으로 제공한다. 코드 수정만으로 바꿀 수 있는 사용자 노출 정보가 남아 있으면 기능 미완료로 판정한다.
